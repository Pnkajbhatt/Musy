  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (responseText) {
        ...
        catch { throw new Error(`Registration failed (${response.status})`); }
      }
  ...
  return (
    <section className="glass w-full max-w-md rounded-3xl p-6 sm:p-9">
      <div className="mb-8">
        ...
        <h1 className="font-display mt-3 text-3xl text-egg-50">Create your account</h1>
        <p className="mt-2 text-sm text-ink-300">Build your listening space and share what you make.</p>
      </div>
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <label className="block text-sm font-medium text-mist-200">
          ...
          <input type="email" name="email" placeholder="you@example.com"
            value={formData.email} onChange={handleChange} className="field" required />
        </label>
        <label className="block text-sm font-medium text-mist-200">
          Password
        ...
        <label className="block text-sm font-medium text-mist-200">
          ...
          <input type="password" placeholder="Repeat your password"
            ...
            onChange={handleChange} className="field" required />
        </label>
